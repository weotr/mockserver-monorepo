...
### Added
- **Java client breakpoint and replay methods** — `MockServerClient` now exposes typed methods for the breakpoint and replay control-plane endpoints: `listBreakpoints()`, `continueBreakpoint(id)`, `modifyBreakpoint(id, request)`, `abortBreakpoint(id)` / `abortBreakpoint(id, response)`, and `replay(request)`. These wrap the existing REST endpoints (`GET/PUT /mockserver/breakpoint/*`, `PUT /mockserver/replay`) using the same Jackson/serializer plumbing as other control-plane methods.
- **Dashboard Breakpoints panel** — a new Breakpoints view in the dashboard lists requests paused by breakpoint expectations, showing method, path, age, and exchange ID. Each paused exchange can be continued unchanged, modified (edit the request JSON before forwarding), or aborted (return without forwarding). The panel polls `GET /mockserver/breakpoint` every 2 seconds while open.
