...
Verify:
```bash
dotnet add package MockServer.Testcontainers --version 7.4.0
...
The NuGet package version tracks the MockServer release (e.g. `7.4.0`); the
...
- NuGet package ID: `MockServer.Testcontainers` (namespace/assembly stay `Testcontainers.MockServer`); version in this release: `7.4.0`.
